...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'upload_document') {
        try {
            ...
            $required_fields = ['company_name', 'contact_no', 'tax_id', 'trade_license', 'lc_number', 'container_name', 'batch_number', 'invoice_id', 'address'];
            ...
            $importer_id = $_SESSION['user_id']; // Correctly use user_id from session
            ...
            $stmt = $pdo->prepare(
                'INSERT INTO importer_documents (importer_id, company_name, contact_no, tax_id, trade_license, lc_number, container_name, batch_number, invoice_id, address, document_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
            );
            $stmt->execute([
                $importer_id,
                $_POST['company_name'],
